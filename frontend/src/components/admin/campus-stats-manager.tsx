...
import { Plus, Edit, Trash2, BarChart3, Users, GraduationCap, Trophy, BookOpen, Building, Award, Star, Target, TrendingUp } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const iconOptions = [
  { value: "Users", label: "Users", component: Users, gradient: "from-blue-500 to-purple-500" },
  { value: "GraduationCap", label: "Graduation Cap", component: GraduationCap, gradient: "from-green-500 to-teal-500" },
  { value: "Trophy", label: "Trophy", component: Trophy, gradient: "from-yellow-500 to-orange-500" },
  { value: "BarChart3", label: "Bar Chart", component: BarChart3, gradient: "from-indigo-500 to-purple-500" },
  { value: "BookOpen", label: "Book Open", component: BookOpen, gradient: "from-emerald-500 to-cyan-500" },
  { value: "Building", label: "Building", component: Building, gradient: "from-gray-500 to-slate-600" },
  { value: "Award", label: "Award", component: Award, gradient: "from-pink-500 to-rose-500" },
  { value: "Star", label: "Star", component: Star, gradient: "from-amber-500 to-yellow-500" },
  { value: "Target", label: "Target", component: Target, gradient: "from-red-500 to-pink-500" },
  { value: "TrendingUp", label: "Trending Up", component: TrendingUp, gradient: "from-lime-500 to-green-500" }
];
...
export function CampusStatsManager() {
  ...
  const fetchStats = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Stats array to set:', statsArray);
        setStats(Array.isArray(statsArray) ? statsArray : []);
      } else {
        const errorText = await response.text();
        console.error('API Error:', response.status, errorText);
        throw new Error(`Failed to fetch campus statistics: ${response.status}`);
      }
    } catch (error) {
      console.error('Error fetching stats:', error);
      toast({
        title: "Error",
        description: "Failed to fetch campus statistics. Please check console for details.",
        variant: "destructive",
      });
  ...
  const getIconGradient = (iconName: string) => {
    const iconOption = iconOptions.find(option => option.value === iconName);
    return iconOption ? iconOption.gradient : "from-blue-500 to-purple-500";
  };
  ...
  console.log('Rendering with stats:', stats, 'Length:', stats.length);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {stats && stats.length > 0 ? stats.map((stat) => {
          try {
            ...
            const iconGradient = getIconGradient(stat?.icon || "Users");
            return (
            <Card key={stat.id} className="relative">
              <CardHeader className="pb-2">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                    <div className={`p-2 rounded-lg bg-gradient-to-r ${iconGradient} shadow-lg`}>
            ...
            console.error('Error rendering stat:', error, stat);
            return null;
          }
        }) : null}
      </div>
