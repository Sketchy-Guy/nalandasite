...
import { Home, Users, MapPin, Phone, Wifi, Car, Utensils, Shield, User } from 'lucide-react';
...
const Hostel = () => {
  ...
  const fetchHostels = async () => {
    try {
      const response = await api.hostels.list({ is_active: true, ordering: 'name' });
                      ...
                      <Card className="group hover:shadow-lg transition-shadow h-full">
                        ...
                        <CardContent className="space-y-6">
                          ...
                          {hostel.images && hostel.images.length > 0 && (
                            <div className={`grid gap-2 ${hostel.images.length === 1 ? 'grid-cols-1' :
                              hostel.images.length === 2 ? 'grid-cols-2' :
                                hostel.images.length === 3 ? 'grid-cols-3' :
                                  'grid-cols-2'
                              }`}>
                              {hostel.images.slice(0, 4).map((image, idx) => (
                                <div
                                  key={image.id}
                                  className={`${hostel.images.length === 4 && idx === 0 ? 'col-span-2' : ''
                          ...
                          {hostel.rules && (
                            <div>
                              ...
                              <div className="bg-muted/50 p-3 rounded-lg text-sm text-muted-foreground whitespace-pre-line">
                                {hostel.rules}
                              </div>
                            </div>
                          )}
